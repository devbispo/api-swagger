...
const requerimentos = [
	{
		...
		situacao: 'Deferido',
		argumentacao:
			'A questão 1 foi respondida conforme o material da disciplina.',
		parecer:
			'A pontuação da questão foi revisada e a nota da prova atualizada.',
	},
	{
		disciplina: { codigo: 'ADS032', nome: 'Construção de Frontend' },
		periodo: '2021/1',
...
app.get('/boletos/:id', function (req, res, next) {
	...
	if (id > boletos.length)
		return next(createError(404, 'Boleto não localizado!'));
	res.json(boletos[id]);
});
...
app.get('/requerimentos/:id', function (req, res, next) {
	const id = Number(req.params.id);
	if (id > requerimentos.length)
		return next(createError(404, 'Requerimento não localizado!'));
